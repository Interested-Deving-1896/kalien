export const COORDINATOR_OBJECT_NAME = "global-proof-coordinator";
export const TESTNET_NETWORK_PASSPHRASE = "Test SDF Network ; September 2015";
export const DEFAULT_BINDINGS_RPC_URL = "https://soroban-testnet.stellar.org";
export const OPENZEPPELIN_CHANNELS_HOSTNAME = "channels.openzeppelin.com";
export const DEFAULT_RELAYER_REQUEST_TIMEOUT_MS = 30_000;

export const TAPE_MAGIC = 0x5a4b5450;
export const TAPE_VERSION = 4;
export const TAPE_HEADER_SIZE = 16;
export const TAPE_FOOTER_SIZE = 8;

export const DEFAULT_MAX_TAPE_BYTES = 2 * 1024 * 1024;
export const DEFAULT_POLL_INTERVAL_MS = 3_000;
export const MIN_PROVER_POLL_INTERVAL_MS = 500;
export const DEFAULT_PROVER_REQUEST_TIMEOUT_MS = 30_000;
export const DEFAULT_PROVER_HEALTH_CACHE_MS = 30_000;
// End-to-end job lifetime cap in the gateway (queue + proving + storage writes).
// Also used as the max time a queued VAST fallback job can wait for a slot.
// Keep this above the full four-attempt fallback chain budget
// (Boundless + Vast + Boundless + Vast) so wall timeout does not preempt
// backend assurance under normal operation.
export const DEFAULT_MAX_PROOF_TOTAL_WALL_TIME_MS = 180 * 60_000; // 3 hours
// Max time a single active prover run should occupy the slot in the gateway.
// Keep slightly above the prover API timeout so status propagation can finish.
export const DEFAULT_MAX_PROVER_RUN_TIME_MS = 11 * 60_000; // 11 minutes
export const DEFAULT_COMPLETED_JOB_RETENTION_MS = 7 * 24 * 60 * 60_000; // 7 days
export const MAX_TOTAL_PROVER_ATTEMPTS = 4; // 2 Boundless + 2 Vast.ai interleaved

export const DEFAULT_SEGMENT_LIMIT_PO2 = 21;

// With a ~10 minute proving SLA, a 5 minute retry backoff just wastes time.
// Keep retries snappy so transient network/prover issues either recover quickly or fail fast.
export const MAX_RETRY_DELAY_SECONDS = 60;

// Must match the max_retries value in wrangler.jsonc queue consumer config.
// After this many delivery attempts (attempts >= MAX_QUEUE_RETRIES), the job is marked
// as permanently failed rather than retried again.
export const MAX_QUEUE_RETRIES = 10;
export const MAX_VAST_QUEUE_RETRIES = 30;
export const DISPATCH_LEASE_TIMEOUT_MS = 60_000;

// When the VastAI slot is occupied, retry the queue message after this delay.
export const VAST_SLOT_BUSY_RETRY_DELAY_SECONDS = 30;
export const EXPECTED_RULES_TAG = 4; // "AST4"
export const EXPECTED_RULES_DIGEST = 0x41535434; // "AST4"
export const EXPECTED_RULESET = "AST4";

// Leaderboard sync defaults (intentionally code-level to keep wrangler vars minimal)
export const DEFAULT_LEADERBOARD_SYNC_MAX_PAGES = 5;
export const DEFAULT_LEADERBOARD_FORWARD_REPLAY_WINDOW_LEDGERS = 8_000;
export const DEFAULT_LEADERBOARD_TAPE_BACKFILL_MAX_PASSES = 4;
export const DEFAULT_LEADERBOARD_TAPE_BACKFILL_BATCH_SIZE = 100;
export const DEFAULT_LEADERBOARD_TAPE_BACKFILL_MAX_BATCHES = 20;
export const DEFAULT_LEADERBOARD_TAPE_BACKFILL_JOBS_PAGE_SIZE = 200;
export const DEFAULT_LEADERBOARD_TAPE_BACKFILL_MAX_JOBS_PER_CLAIMANT = 5_000;
export const DEFAULT_LEADERBOARD_TAPE_BACKFILL_OLDEST_FIRST = false;
export const DEFAULT_LEADERBOARD_TAPE_STALE_PRUNE_BATCH_SIZE = 40;
export const DEFAULT_LEADERBOARD_TAPE_STALE_PRUNE_MAX_BATCHES = 10;
export const DEFAULT_LEADERBOARD_TAPE_STALE_PRUNE_OLDEST_FIRST = true;

export const RETRYABLE_JOB_ERROR_CODES = new Set([
  "server_restarted",
  "proof_error",
  "internal_error",
]);

// Claim auto-retry defaults (maintenance-driven recovery for failed claims)
export const MAX_CLAIM_AUTO_RETRIES = 20; // Total claim attempts (queue + auto) before giving up
export const CLAIM_AUTO_RETRY_COOLDOWN_MS = 5 * 60_000; // 5 min between auto-retries

// Claim artifact fetch retry (R2 transient consistency)
export const CLAIM_ARTIFACT_READ_MAX_ATTEMPTS = 3;
export const CLAIM_ARTIFACT_READ_RETRY_DELAY_MS = 1_000;

// Boundless proving defaults
export const DEFAULT_BOUNDLESS_POLL_TIMEOUT_MS = 30 * 60_000; // Full lock window: flat (1m) + lockTimeout (29m from rampUpStart)
export const DEFAULT_BOUNDLESS_MAX_FRAMES = 36_000;
