...
1. Verifies a RISC Zero proof through the on-chain verifier.
...
- `Admin`, `VerifierId`, `ImageId`, `TokenId`, `Paused` are in instance storage.
...
9. Call verifier verify with `(seal, image_id, journal_digest)`.
...
- `set_paused(paused)`
- `set_verifier_id(new_verifier_id)`
