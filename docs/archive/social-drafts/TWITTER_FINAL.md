...
Of course I had to get busy exploring, understanding, implementing, and experimenting. The Stellar universe has become so unbelievably rich and diverse over the past few years. What you can now tie together and dream up and implement is nearing infinity, and with the assistance of AI to develop, the speed of experimentation has never been faster.
...
A lot of ZK projects are about privacy, payments, and obfuscation. But I think some of the best ways to understand something — or begin to explore it — are actually through games. Games can be incredibly complex. And when you're talking about things like points, scores, and leaderboards, the security becomes paramount. Fair, compelling, addictive gameplay where players keep returning to beat their own or others' high scores. That seemed like a perfect place to try an experiment with ZK.

### Why a game needs ZK

A leaderboard says some guy scored 84,390 points. Cool. Says who?
...
That means a score is no longer just a claim. It's the output of a replayable state machine.

The ZK part codifies the rules of Asteroids into a RISC Zero virtual machine that produces a proof that Stellar, after Protocol 25, is now able to verify. The end-to-end flow:
...
If all that passes, and only then, the contract mints your score as brand new KALIEN. Once that proof is verified, it's safe to say: this score is fair. I don't know who generated it. I don't know how it was generated. But I am absolutely certain that it was played fairly. Whatever the score is, that's what I'll mint.

All of this compute — 60 frames per second of a complex physics-based shooter — is getting proven offchain in a way that's now safe to make rewards onchain. That's amazing. But it has so many implications beyond just gaming.

### Why Stellar, and why now

Protocol 25, code-named X-Ray, added native BN254 host functions plus Poseidon and Poseidon2 hash primitives. This is the sort of plumbing you need if you want proof systems to feel native instead of bolted on.
...
Poseidon and Poseidon2 give developers hash primitives that are far more efficient inside arithmetic circuits than dragging SHA-256 or Keccak through a zk pipeline. If you're building proof systems that touch Stellar, these are the hashes you want.

KALIEN is one of the first production applications exercising these capabilities on mainnet. Not in theory. In a system people are actively competing in right now.
...
If you're unfamiliar with the KALE project, that was one I did a couple years ago exploring some other areas of Stellar. It's a community proof-of-teamwork mining token — farmers run CPU/GPU miners, plant, work, harvest — and it's kind of become a meme in the protocol and in the ecosystem. The 100-millionth Soroban smart contract transaction on Stellar mainnet was a KALE plant invocation, if that gives you any sense of the activity.

The name KALIEN definitely comes from tying Asteroids and KALE aliens together. It's a fun way to connect these two things.

KALIEN tokens aren't just a number on a leaderboard. There's a live KALE/KALIEN liquidity pool on Soroswap seeded with 40 million KALE and 4 trillion KALIEN, sitting at a ratio of roughly 100,000 KALIEN per 1 KALE. The web app has a built-in swap so you can convert KALIEN to KALE directly from your wallet.
...
One thing I care a lot about is UX, and the worst possible version of this project would be one where you have to install a browser extension and manage a seed phrase before you can play a game.
...
I wanted to encourage the use of CLI interactions where bots are playing games of Asteroids. The idea being that bots would try to max score. I wanted to ensure that no matter whether you played as a human or a bot, everybody was guaranteed to be playing by the same set of rules.
...
I strongly encourage everyone to go play the game and build their own CLI farmer.

### People are already pushing the ceiling

A good example of how this plays out: the average person could probably score about 50,000 points before dying, before it becomes too intense. There are already scores of over a million on the leaderboard. Bots are running algorithms that way outperform humans. They're all playing by the exact same set of rules, which is awesome to me.
...
You swap out games for other complex compute tasks and the same pattern applies. The chain only has to do a minimal amount of work — transferring an asset, minting a token, authorizing an action. The chain can do the good chain things that it does, like the distributed ledger, and the offchain world can do the complex compute. Things that just could not have safely been done before are now relatively trivial:
...
This is an innovation that entirely shifts what blockchains can be used for. And it's particularly exciting when you're AI-assisted as an engineer. You can write the proof with AI, you can write the interface with AI, you can write a lot of the chain logic, contracts, and wallet infrastructure with AI. Write really good tests and then ensure that those tests hold as you continue to develop the system. We're in an amazing time to begin crafting entirely new interactions and systems that are useful and impactful.

By being able to have this combination — distributed ledger technology for balance reconciliation, complex computation happening on isolated singular machines, and proofs that verify the computation produced consistent results — we're looking at something a lot more developers need to start diving into.

### Go play it

KALIEN is an actual thing you can touch, not a diagram in a pitch deck.

You can play it. You can submit runs. You can watch proof jobs process. You can inspect the leaderboard. You can read the contract. You can swap KALIEN for KALE. You can run the CLI and turn your machine into a farm. You can explore the repo and start mixing and matching, stealing from there. There's a lot of tech behind the scenes that actually pulls this off, and it'll be very useful to folks building on ZK, building on RISC Zero, building on Stellar.

It's a good entry point for people who are new to blockchain or starting to dive into zero-knowledge systems or smart wallet systems or combining real-world assets with complex offchain compute. There's a lot of experiments that need to be run, a lot of work that needs to be done.

Play around, share it around, enjoy, and let me know what your high score is.
