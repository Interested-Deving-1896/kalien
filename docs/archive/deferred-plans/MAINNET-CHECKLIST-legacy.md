...
- [ ] **Set a strong `API_KEY`** on the prover instance. The prover currently
      ...
      (`kalien-verifier/api-server/src/main.rs`). Generate a random secret (32+ chars)
...
- [ ] **Set `PROVER_API_KEY` on the Cloudflare Worker** via
      ...
      the prover.
- [ ] **Restrict CORS on the prover**. Configure `CORS_ALLOWED_ORIGIN` on
      `kalien-verifier/api-server` to the worker origin (or keep CORS disabled
      and expose the prover only behind Cloudflare Tunnel/Access).
...
- [ ] **Add frontend environment variables** for mainnet. Use current frontend
      config in `.env.example` as baseline and set production values:
      ```
      ...
      VITE_RELAYER_PROXY_URL=<worker relay endpoint>
...
- [ ] **Run cross-implementation parity tests**. Play back the same tape in
      ...
      `asteroids-core` — final scores and checksums must match
      exactly.
...
- [ ] **Verify no secrets are committed and `.example` files exist** for every
      ...
      - `.dev.vars.example` exists for Worker-side secret/config docs
      - `kalien-contract/.env.example` exists for contract script state/env docs
      - `kalien-verifier/api-server/.env.example` exists (good)
      - Root `.gitignore` ignores `.env` and related local state (verify before release)
      - `kalien-verifier/.gitignore` exists (verify local env patterns are still adequate)
