# RISC0 Asteroids Verifier Status (AST4)
...
- Strict fairness: verification must prove the game was played according to the AST4 rules as implemented by the verifier (no "trust the client" outcomes).
- Determinism: the replay must be fully deterministic (no floats, no host timing, no nondeterministic iteration).
- Forward‑only tape format: AST4 v4 tapes only (16-byte header, nibble-packed body, 8-byte footer; claimant not embedded in tape; legacy tapes are rejected).
...
2. **Deterministic replay of outcomes**
   - The verifier replays from `seed + inputs` and computes `final_score` and `frame_count`.
